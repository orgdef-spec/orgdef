...
**Created:** 2026-04-30
**Target version:** canonical-orgs `oagp-family-open-standard` v1.1.0 + orgdef CONTRIBUTING placement-convention guidance
...
Bundles refinements to the `oagp-family-open-standard` canonical template based on three independent derivation experiences, plus a filesystem-convention change for operational orgdef placement that preserves cross-spec aggregation. All template revisions are content patches (no schema changes); the placement convention is a CONTRIBUTING-level documentation update plus a one-time migration of four spec repos (orgdef-spec, memodef-spec, roledef-spec, catdef-spec).
...
```
"Part of the Open Agentic Governance Pattern (OAGP) family — alongside catdef, roledef, orgdef, and memodef — defining the pattern of [SLOT: what aspect of AI-first organization this spec addresses]. The primary reader of [SLOT: this spec's primary artifact type] is an AI; [SLOT: any spec-specific framing of the cross-org or cross-runtime use case]."
```
...
```
"vision_first_clause": "Part of the Open Agentic Governance Pattern (OAGP) family ... defining the pattern of [SLOT: noun-phrase].",
...
```
"description": "[SLOT: spec's primary artifact] MUST be sufficient — alone or composed with other OAGP-family artifacts — for a fresh AI runtime to act on its content with zero context backfill."
```
...
Older specs (catdef, predating OAGP-family naming conventions) use seat names like "Chief Strategist" or "steward" rather than the canonical's `senior-open-standards-strategist`. Acknowledge in instantiation_notes:
...
> REPLACE for mission, vision, and scope still expects family-resemblance in shape. Derivers' REPLACE artifacts SHOULD echo the canonical's structural framing (zero-context-backfill, OAGP-family identification, primary-reader-is-AI) even when the noun-phrases differ.
...
**Rejected because:** the four OAGP-family operational orgdefs are landing now; future derivers (e.g., when catdef.org PRs reference the canonical) will be guided by the v1.1 form. Delaying means new derivers either learn the wrong shape from v1.0.0 or wait for v1.1 to land.
...
1. **Canonical template patched** at `orgdef-spec/orgdef/proposed-orgs/oagp-family-open-standard.openthing` to v1.1.0 — all P1–P9 patches applied, instantiation_notes updated
